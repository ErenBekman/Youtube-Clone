        ...
        <v-col cols="11" class="mx-auto">
          <v-row>
            <v-col cols="12" sm="12" md="8" lg="8">
              ...
              >
                <v-responsive>
                  <video-player :src="video?.videoUrl" />
                </v-responsive>
                <v-card flat tile class="card">
                  ...
                  <div class="d-flex flex-wrap justify-space-between" id="btns">
                    ...
                    >
                      ...
                      >{{ video?.createdAt | formatDateHoursAgo }}
                    </v-card-subtitle>
                    <v-card-actions class="pt-0 pl-0 grey--text">
                      <v-btn
                        text
                        @click.prevent="likeVideo"
                        :disabled="likeButton"
                ...
                <v-row class="justify-space-between">
                  <v-col cols="6" sm="6" md="5" lg="5">
                    <v-card class="transparent" flat>
                      <v-list-item three-line>
                        <nuxt-link :to="`/Channels/${video?.user_id?._id}`">
                        ...
                        <v-list-item-content class="align-self-auto">
                          <v-list-item-title class="font-weight-medium mb-1">{{
                            video?.user_id?.name || "channel_name"
                          }}</v-list-item-title>
                          <v-list-item-subtitle>
                            {{ video?.user_id?.subscribers }} subscribers
                          </v-list-item-subtitle>
                        </v-list-item-content>
                      </v-list-item>
                    </v-card>
                  ...
                  <v-col>
                    <p class="mb-0">{{ comments?.length || 0 }} Comments</p>
                        ...
                        <v-list-item-content class="align-self-auto">
                          ...
                          >
                            ...
                            <v-btn
                              ...
                              depressed
                              tile
                              :disabled="commentData.description === ''"
                    ...
                    >
                      <v-list-item three-line class="pl-0 mt-2">
                        <nuxt-link :to="`/Channels/${item?.user_id?._id}`">
                          <v-list-item-avatar size="50"
                            ><v-img
                              src="https://randomuser.me/api/portraits/men/1.jpg"
                            ></v-img
                        ...
                        <v-list-item-content>
                          <v-list-item-title
                            ...
                            <span class="font-weight-light grey--text">
                              {{ item?.createdAt | formatDateHoursAgo }}
...
export default {
  ...
  methods: {
    ...
    async likeVideo() {
      await this.$axios
        .$put(`/api/users/like/${this.$route.params.id}`)
        .then(() => {
    ...
    async dislikeVideo() {
      await this.$axios
        .$put(`/api/users/dislike/${this.$route.params.id}`)
        .then(() => {
          this.dislikeButton = true;
          this.video.dislikes.length++;
        });
    },
    showReply(id) {
  ...
  mounted() {
    ...
    this.commentData.video_id = this.$route.params.id;
  },
};
</script>
